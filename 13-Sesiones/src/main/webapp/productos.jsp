...
<body>
	...
	<div class="carrito">
		<div class="filaPrecio">
			<p class="etiqueta etiquetaCarrito">Carrito:</p>
			<div>
		...
		if (session.getAttribute("produ") != null) {
			...
			for (int i = 0; i < listaProductos.size(); i++) {
				...
				"<div class='productoCarrito'><div class='columna'><p class='etiqueta'>Producto:</p><p class='producto'>"
						...
						+ listaProductos.get(i).getPvp()
						+ "</p></div><div class='columna'><p class='etiqueta'> Cantidad:</p><p class='pvp'>"
						+ listaProductos.get(i).getCantidad()
						+ "</p></div><div class='columna columna4'><form action='RecepcionFormulario' method='post'><input class='contenedor boton' type='submit' value='Eliminar 1' /><input type='hidden' name='restarProducto' value='"
						+ listaProductos.get(i).getIdproducto()
						+ "' /></form><form action='RecepcionFormulario' method='post'><input class='contenedor boton' type='submit' value='Eliminar' /><input type='hidden' name='eliminarProducto' value='"
						+ listaProductos.get(i).getIdproducto() + "' /></form></div></div>");
			}
		}
		%>
	</div>
	<div class="productos">
		<form action="RecepcionFormulario" method="post">
			...
			<div class="fila2">
				<div class="contenedor descripcion">
					...
					<p class="descripcion">Presentando este mouse Vendetta de
						Forgeon puedo decirte que te encuentras ante el ratón más
						competente del mercado. Este mouse ha sido por varios años
						consecutivos elegido el mejor mouse gaming en cuanto a su diseño.
						Dispone de diferentes botones que te generan todos los atajos y
						prestaciones que necesites al alcance de tu mano y lleva también
						un sensor que regula la velocidad que quieras en cada momento. Con
						el mouse Vendetta lo ganarás todo.</p>
				</div>
			</div>
			<input type="hidden" name="idProductoH" value="1" /> <input
				type="hidden" name="stockActualH" value="50" /> <input
				type="hidden" name="stockMinimoH" value="5" /> <input type="hidden"
				name="productoH" value="raton" /> <input type="hidden"
				name="descripcionH"
		...
		<form action="RecepcionFormulario" method="post">
			...
			<div class="fila2">
				<div class="contenedor descripcion">
					<p class="etiqueta">Descripcion:</p>
					<p class="descripcion">Combina lo mejor de ambas tecnologías
						para darte una sensación de uso muy superior al resto, la
						tecnología Híbrida ha llegado para quedarse y este teclado es
						buena muestra de ello. Además, en Tempest no se quedan cortos en
						cuanto a iluminación y cuenta con hasta 13 modos de iluminación
						RGB diferente tanto en las teclas como alrededor del teclado, lo
						cual aporta una sensación ultra gaming. Tu teclado brillará como
						nunca has visto nada igual. Pero esto no es todo, Tempest piensa
						cada detalle de su producto y gracias a su reposamuñecas
						acolchado, su uso será realmente cómodo, permitiendo un uso
						prolongado si que acuses cansancio o desgaste. No nos equivocamos
						si decimos que no has visto un teclado igual, podrás ponerlo a
						juego con tu setup y se convertirá en tu mejor escudero durante la
						batalla. Si todo esto te parece poco, el teclado está fabricado en
						aluminio de alta calidad lo cual aporta un extra de durabilidad y
						una sensación premium, que hará que tu teclado dure mucho más
						tiempo y resista todo tipo de uso diario.</p>
				</div>
			</div>
			<input type="hidden" name="idProductoH" value="2" /> <input
				type="hidden" name="stockActualH" value="27" /> <input
				type="hidden" name="stockMinimoH" value="10" /> <input
				type="hidden" name="productoH" value="teclado" /> <input
				type="hidden" name="descripcionH"
		...
		<form action="RecepcionFormulario" method="post">
			...
			<div class="fila2">
				<div class="contenedor descripcion">
					<p class="etiqueta">Descripcion:</p>
					<p class="descripcion">Nuestra serie de monitores gaming Icarus
						da la bienvenida al modelo de 27” QHD 144Hz R1500 VA - IC27Q4-V2P.
						Este nuevo monitor de Newskill está diseñado para aquellos
						jugadores exigentes en cuanto a resolución y calidad de imagen.
						Con una interesante tasa de refresco de hasta 144Hz, resolución
						2560x1440P “2K” y compatible tanto con G-Sync como con FreeSync,
						nuestro nuevo Icarus es lo que necesitas para llevar tu set-up al
						siguiente nivel.</p>
				</div>
			</div>
			<input type="hidden" name="idProductoH" value="3" /> <input
				type="hidden" name="stockActualH" value="18" /> <input
				type="hidden" name="stockMinimoH" value="10" /> <input
				type="hidden" name="productoH" value="monitor" /> <input
				type="hidden" name="descripcionH"
		...
		<form action="RecepcionFormulario" method="post">
			...
			<div class="fila2">
				<div class="contenedor descripcion">
					<p class="etiqueta">Descripcion:</p>
					<p class="descripcion">El día que esperabas ha llegado y
						Tempest GHS301 Barbarian estarán contigo en esa batalla. Tempest
						sube de nivel con los auriculares gaming definitivos. Estos
						auriculares disponen de unos altavoces 7.1 virtual de alta calidad
						estéreo con materiales de última generación, llevando el estatus
						de nuestra marca a un nuevo nivel.</p>
				</div>
			</div>
			<input type="hidden" name="idProductoH" value="4" /> <input
				type="hidden" name="stockActualH" value="85" /> <input
				type="hidden" name="stockMinimoH" value="20" /> <input
				type="hidden" name="productoH" value="auriculares" /> <input
				type="hidden" name="descripcionH"
		...
		<form action="RecepcionFormulario" method="post">
			...
			<div class="fila2">
				<div class="contenedor descripcion">
					<p class="etiqueta">Descripcion:</p>
					<p class="descripcion">Bose Companion 2 Series III Altavoces
						multimedia. ¿Te encanta reproducir música, juegos y vídeos en tu
						ordenador? ¡Actualiza tu sonido y tu experiencia! Hazlo con el
						sistema de altavoces multimedia Companion 2 Series III.
						Disfrutarás de su rendimiento, que es considerablemente mejor que
						el de tus altavoces originales: y es nuestro sistema de altavoces
						para ordenador más asequible.</p>
				</div>
			</div>
			<input type="hidden" name="idProductoH" value="5" /> <input
				type="hidden" name="stockActualH" value="15" /> <input
				type="hidden" name="stockMinimoH" value="5" /> <input type="hidden"
				name="productoH" value="altavoces" /> <input type="hidden"
				name="descripcionH"
