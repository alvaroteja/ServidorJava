...
<body>
	...
	<form name="AnyadirForm" action="shopping" method="POST">
		<input type="hidden" name="todo" value="add"> T´ıtulo: <select name=idLibro>
			<%
			// Scriptlet 1: Carga los libros en el select.
			for (int i = 0; i < LibrosBD.tamanyo(); i++) {
				String stock = "";
				if (LibrosBD.getStock(i) <= 10) {
					stock = " | Quedan " + LibrosBD.getStock(i) + "!";
				}
				out.println("<option value=" + i + ">");
				out.println(LibrosBD.getTitulo(i) + " | " + LibrosBD.getAutor(i) + " | " + LibrosBD.getPrecio(i) + stock);
				out.println("</option>");
			}
			%>
		</select> Cantidad: <input type="text" name="cantidad" size="10" value="1"> <input type="submit" value="Añadir a la cesta">
	</form>
	<br />
	<hr />
	<br />
	...
	<form name="checkoutForm" action="shopping" method="POST">
		<input type="hidden" name="todo" value="checkout"> <input type="submit" value="Checkout">
	</form>
	<%
	} // if
	%>
